...
class MPRNet(nn.Module):
    ...
    def forward(self, x3_img):
        ...
        #     torch.pow(x3_img[:, 0, :, :], 2.2) +
        #     torch.pow(x3_img[:, 1, :, :]* 1.5, 2.2) +
        #     torch.pow(x3_img[:, 2, :, :]* 0.6, 2.2),
        #     1.0 / 2.2
        # )
        # luminance = luminance.unsqueeze(1)
        luminance =  (x3_img[:, 0, :, :]*0.299) + (x3_img[:, 1, :, :]* 0.587) +(x3_img[:, 2, :, :]* 0.114)+16

        thresold = 1-self.regressor(luminance)
        
        x_mask = torch.zeros((b, 512, 512),dtype=x3_img.dtype,device = x3_img.device)
        for i in range(b):
        ...
        deflare_img=torch.cat((y[:,:3,:,:],x0),dim=1)
