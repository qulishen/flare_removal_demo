...
class Uformer(nn.Module):
    def __init__(self, img_size=128, img_ch=3,output_ch=3,
        ...
        self.decoderlayer_3 = BasicUformerLayer(dim=embed_dim*2,
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,se_layer=se_layer)
        self.choice_conv = nn.Sequential(nn.Conv2d(out_chans,int(out_chans/2),kernel_size=1))
    ...
    def forward(self, x, mask=None):
        ...
        luminance =  (x[:, 0, :, :]*0.299) + (x[:, 1, :, :]* 0.587) +(x[:, 2, :, :]* 0.114)+16
        ...
        # print("asdasdasd")
        # print(device_id)
        x_mask = x * x_mask.to("cuda:"+str(device_id)) 
        
        # Adaptive Focus Module.
    
        y = self.input_proj(x_mask)
        y = self.pos_drop(y)
        #Encoder
        ...
        deflare_img=torch.cat((y[:,:3,:,:],x),dim=1)
